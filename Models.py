...
class TextRNNAttention(tf.keras.Model):
    def __init__(self, config, VOCAB_SIZE):
        ...
        self.word_forward_LSTM = tf.keras.layers.LSTM(self.config.RNN_DIM, return_sequences=True)
        self.word_backward_LSTM = tf.keras.layers.LSTM(self.config.RNN_DIM, return_sequences=True, go_backwards=True)

        self.dense1 = tf.keras.layers.Dense(self.config.HIDDEN_DIM)

        self.Ws = [tf.Variable(shape=[self.config.RNN_DIM*2, self.config.DIM1], dtype=tf.float32) for t in range(self.config.MAX_LENGTH)]

        self.tanh = tf.keras.activations.tanh()

        self.dense1 = tf.keras.layers.Dense(self.config.SENTENCE_DIM)
        self.sent_forward_LSTM = tf.keras.layers.LSTM(self.config.RNN_DIM, return_sequences=True)
        self.sent_backward_LSTM = tf.keras.layers.LSTM(self.config.RNN_DIM, return_sequences=True, go_backwards=True)

        self.S = tf.Variable(shape=[self.config.SENTENCE_DIM*2, self.config.HIDDEN_DIM], dtype=tf.float32)

    def call(self, inputs, training=None, mask=None):
        embed = tf.transpose(self.embedding(inputs), perm=[1, 0, 2])

        word_forward_out = self.forward_LSTM(embed)
        word_backward_out = self.backward_LSTM(embed)

        # [seq_L, batch_size, HIDDEN_DIM]
        word_out = self.dense1(tf.concat([word_forward_out, word_backward_out], axis=-1))

        sent_inputs = []
        for t in range(self.config.MAX_LENGTH):
            out = self.tanh(tf.scan(lambda a, X: tf.linalg.matmul(X, self.Ws[t]), word_out))
            sent_inputs.append(out)

        self.dropout = tf.keras.layers.Dropout(0.5)

        self.dense1 = tf.keras.layers.Dense(self.config.HIDDEN_DIM,
                                            input_shape=(None, self.config.RNN_DIM * 2 + self.config.EMBEDDING_DIM),
                                            activation='tanh')
