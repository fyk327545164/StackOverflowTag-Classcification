...
with open('gcp_title.txt', 'w') as f:
    ...
    for i in range(1, 17914 // 50):
        ...
        url = "https://stackoverflow.com/questions/tagged/google-cloud-platform?tab=newest&page=" + str(i) + "&pagesize=50"
        ...
        try:
            ...
            for item in items:
                ...
                # commit_detail([item[item.index('>') + 1:item.index('</')],
                #                base_url + item[item.index('"/') + 1:item.index('" c')]], q_f, a_f, num, "AZURE")
